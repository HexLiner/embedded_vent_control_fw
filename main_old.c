...
typedef union {
    struct {
        ...
        uint8_t min_temperature_c;
        uint8_t max_temperature_nm_c;
        uint8_t max_temperature_m_c;
        uint8_t max_heating_time_minutes;
        uint8_t max_heating_pause_time_minutes;
        uint8_t max_vent_time_minutes;
        uint8_t max_vent_pause_time_minutes;
    };
    uint8_t config[9];
} device_config_t;
...
timer_t heating_timer_s, vent_timer_s;
...
int main(void) {
    ...
    heating_timer_s = systimer_set_s(30);

    vent_dis();
    vent_state = VENT_STATE_DISABLE;
    vent_timer_s = systimer_set_s(30);
    ...
    while(1) {
        ...
        switch (heater_state) {
            case HEATER_STATE_DISABLE:
                if (systimer_triggered_s(heating_timer_s)) {
                    if ((curr_temperature_c < device_config.min_temperature_c) || ((curr_humidity_pct > device_config.max_humidity_pct) && (curr_temperature_c < max_temperature_c))) {
                        heating_timer_s = systimer_set_s(device_config.max_heating_time_minutes * 60);
                        heater_en();
            ...
            case HEATER_STATE_ENABLE:
                if (systimer_triggered_s(heating_timer_s) || (curr_temperature_c > max_temperature_c)) {
                    heating_timer_s = systimer_set_s(device_config.max_heating_pause_time_minutes * 60);
                    heater_dis();
                    heater_state = HEATER_STATE_DISABLE;
                }
                break;
        ...
        switch (vent_state) {
            case VENT_STATE_DISABLE:
                if ((systimer_triggered_s(vent_timer_s) && (curr_humidity_pct > device_config.max_humidity_pct)) || is_man_mode) {
                    vent_timer_s = systimer_set_s(device_config.max_vent_time_minutes * 60);
                    vent_en();
            ...
            case VENT_STATE_ENABLE:
                if ((systimer_triggered_s(vent_timer_s) || (curr_humidity_pct < device_config.min_humidity_pct)) && !is_man_mode) {
                    vent_timer_s = systimer_set_s(device_config.max_vent_pause_time_minutes * 60);
                    vent_dis();
                    vent_state = VENT_STATE_DISABLE;
                }
                break;
        ...
        switch (lcd_state) {
            ...
            case LCD_STATE_MAIN_SCREEN:
                ...
                if (heater_state == HEATER_STATE_ENABLE) ssd1306_print_simw('H', 2, 96, 16);
                else ssd1306_print_simw(' ', 2, 96, 16);
                if (vent_state == VENT_STATE_ENABLE) ssd1306_print_simw('V', 2, 108, 16);
            ...
            case LCD_STATE_MENU_ITEM_VALUE:
                if (is_lcd_state_init) {
                    ...
                    y = device_config.config[lcd_menu_curr_item - LCD_MENU_CONFIG_ITEMS_OFFSET];
                }
                ...
                if ((enc_step != 0) || is_lcd_state_init) {
                    ...
                    if (i != y) {
                        device_config.config[lcd_menu_curr_item - LCD_MENU_CONFIG_ITEMS_OFFSET] = y;
                        eeprom_driver_write_8((lcd_menu_curr_item - LCD_MENU_CONFIG_ITEMS_OFFSET), device_config.config[lcd_menu_curr_item - LCD_MENU_CONFIG_ITEMS_OFFSET]);
                    }
                    is_lcd_state_init = true;
                    lcd_state = LCD_STATE_MENU_ITEMS_LIST;
                }
        ...
        if (systimer_triggered_s(log_process_timer_s)) {
            ...
            if (heater_state == HEATER_STATE_ENABLE) {
                log_process_heat_vent_states |= 1 << (log_process_minutes_cnt >> 3);
                statistics_time_heater_en_minutes++;
            }
            if (vent_state == VENT_STATE_ENABLE) {
                log_process_heat_vent_states |= 0x10 << (log_process_minutes_cnt >> 3);
            ...
            if (log_process_minutes_cnt >= 15) {
                log_process_minutes_cnt = 0;

                if (log_process_eeprom_index < (EEPROM_LOG_BASE_ADDR + EEPROM_LOG_SIZE)) {
                    log_process_humidity_avg += curr_humidity_pct;   // 15 + 1 = 16
                    log_process_humidity_avg = log_process_humidity_avg >> 4;  // / 16
